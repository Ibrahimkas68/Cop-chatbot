...
public class PostgresSearchAdapter implements SearchRepository {
    ...
    @Override
    public List<SearchResult> fetchAllDocuments(String tableName, int size) {
        long startTime = System.currentTimeMillis();
        List<SearchResult> results = new ArrayList<>();

        // Determine actual fields to search
        List<String> actualFieldsToSearch = getAllSearchableFields(tableName);
        System.out.println("Fetching all documents from table " + tableName + " with fields: " + actualFieldsToSearch);

        if (actualFieldsToSearch.isEmpty()) {
            System.out.println("No searchable fields found for table " + tableName + ", returning empty results.");
            return results;
        }

        // Build the query to select all documents
        StringBuilder sql = new StringBuilder("SELECT id, ");
        sql.append(String.join(", ", actualFieldsToSearch));
        // Explicitly add url and image_name if they exist in the table
        if (tableHasColumn(tableName, "url")) {
            sql.append(", url");
        }
        if (tableHasColumn(tableName, "image_name")) {
            sql.append(", image_name");
        }
        sql.append(" FROM ").append(sanitizeIdentifier(tableName));
        sql.append(" LIMIT ").append(size); // Apply size limit

        try {
            List<Map<String, Object>> rawResults = jdbcTemplate.queryForList(sql.toString());

            for (Map<String, Object> row : rawResults) {
                SearchResult searchResult = new SearchResult();
                searchResult.setId(String.valueOf(row.get("id")));
                searchResult.setTableName(tableName);

                // Combine content from all searchable fields
                StringBuilder contentBuilder = new StringBuilder();
                for (String field : actualFieldsToSearch) {
                    Object value = row.get(field);
                    if (value != null) {
                        contentBuilder.append(value.toString()).append(" ");
                    }
                }
                searchResult.setDescription(contentBuilder.toString().trim()); // Using description to store combined content

                // Set title if available
                if (row.containsKey("title_fr")) { // Assuming title_fr is a common title field
                    searchResult.setTitle((String) row.get("title_fr"));
                } else if (row.containsKey("title_ar")) {
                    searchResult.setTitle((String) row.get("title_ar"));
                } else if (row.containsKey("title")) { // Check for generic 'title' column
                    searchResult.setTitle((String) row.get("title"));
                }

                // Set URL and ImageName if available
                if (row.containsKey("url")) {
                    searchResult.setUrl((String) row.get("url"));
                } else if (row.containsKey("slug")) { // Construct URL from slug if 'url' column doesn't exist
                    Object slugObject = row.get("slug");
                    if (slugObject instanceof String) {
                        String slug = (String) slugObject;
                        if (!slug.isEmpty()) {
                            // Assuming a base URL structure, e.g., "https://www.e-himaya.gov.ma/"
                            // This base URL might need to be configurable or determined dynamically.
                            // For now, let's use a placeholder.
                            searchResult.setUrl("https://www.e-himaya.gov.ma/" + tableName + "/" + slug);
                        }
                    }
                }
                if (row.containsKey("image_name")) {
                    searchResult.setImageName((String) row.get("image_name"));
                }

                results.add(searchResult);
            }
            System.out.println("Successfully fetched " + results.size() + " documents from table " + tableName);

        } catch (Exception e) {
            System.err.println("Error fetching all documents from table " + tableName + ": " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean tableHasColumn(String tableName, String columnName) {
        try {
            ...
            return jdbcTemplate.queryForObject(sql, Boolean.class, tableName.toLowerCase(), columnName.toLowerCase());
        } catch (Exception e) {
            System.err.println("Error checking for column existence: " + e.getMessage());
            return false;
        }
    }
